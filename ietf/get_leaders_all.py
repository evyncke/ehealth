...
def getGroupFromName(name):
    ...
    print(url)
    groupTree = etree.parse(request.urlopen(url))
    root = groupTree.getroot()
    ...
    for object in objects:
       ...
       return { 'id': object.find('id').text, 'name': object.find('name').text}
...
# Read all groups, possibly not the smartest thing to do but simple enough
